...
namespace DuckovCustomSounds.CustomFootStepSounds
{
    internal static class FootstepConfigLoader
    {
        ...
        {
            var level = DuckovCustomSounds.Logging.LogLevel.Info;
            if (Enum.TryParse(cfg.Debug.Level, true, out DuckovCustomSounds.Logging.LogLevel parsed)) level = parsed;
            // 应用日志配置到FootstepLogger
            FootstepLogger.Configure(cfg.Debug.Enabled, level);
            // 文件快速开关
