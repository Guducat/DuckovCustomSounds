...
## item_sound_map.json 配置（TypeID→类别映射与无事件注入）

本模块支持通过 JSON 配置文件精细控制“物品 TypeID → 音效类别（soundKey）”的映射，以及在“原版没有任何音效事件”的情况下主动注入自定义音效。

- 文件位置：`DuckovCustomSounds/CustomItemSounds/item_sound_map.json`
- 首次运行：若不存在，会自动生成默认文件
...
字段说明：
- defaultWhenNoEvent：字符串，可为 null。当某个物品“没有原版音效事件”且未命中 items 映射时，回退到该类别（如 "bandage"）。
- aliases：对象。类别别名映射。内置将 `meds` 映射为主分类 `bandage`（即原版 use_meds → use_bandage）。
- items：对象。以 TypeID（字符串）为键，配置该物品的策略：
  - soundKey：此物品应使用的类别（如 `bandage`、`syringe`、`food`）。
  - actionKey / finishKey：可为 Action/Finish 两个阶段指定不同类别（可选）。
  - fileBase / actionFileBase / finishFileBase：覆盖文件“基名”，用于多个 TypeID 共享同一份音频（详见下文“高级功能”）。
  - forceWhenNoEvent：bool，默认 true。在“原版完全无事件”时是否主动注入自定义音效。

> 提示：主分类为 `bandage`，`meds` 仅作为别名；ModConfig 中的“绷带/药品”开关统一控制这两者。

### 针对“打针（syringe）”与“吃药/绷带（bandage）”的示例

- 注射器（syringe）：为 TypeID 156、157 指定分类，并为两个阶段指定不同的共享文件基名

```json
{
  "items": {
    "156": { "soundKey": "syringe", "actionFileBase": "needle", "finishFileBase": "needle_done" },
    "157": { "soundKey": "syringe", "actionFileBase": "needle", "finishFileBase": "needle_done" }
  }
}
```
...
对应的文件放置（二选一或者都提供）：
```
DuckovCustomSounds/CustomItemSounds/
└─ syringe/
   ├─ needle_action.mp3
   └─ needle_finish.mp3
```

- 药品/绷带（bandage）：为 TypeID 403 强制注入；并让 23、25 共用同一份文件 `64.mp3`
...
└─ bandage/
   └─ 64.mp3
# 可选分段：64_action.mp3 / 64_finish.mp3
```

### 高级功能说明

- fileBase / actionFileBase / finishFileBase（共享同一音频）
  - 作用：让多个不同 TypeID 指向相同的实际音频文件名（不必为每个 TypeID 复制一份）
  - 示例：`"fileBase":"64"` ⇒ 将尝试 `64.(mp3|wav|ogg|oga)`；阶段化则尝试 `64_action.*` / `64_finish.*`
- forceWhenNoEvent（为“无事件”物品主动注入）
  - 默认 true。若游戏原版在某阶段根本没有发出任何 SFX 事件，模块会在该阶段结束时“兜底注入”配置的音效
  - 若不希望给某个 TypeID 注入，可显式设为 false
- actionKey / finishKey（阶段分类覆盖）
  - 允许在 Action/Finish 阶段使用不同的类别（例如 Action 走 `syringe`，Finish 走 `bandage`）
  - 若相应阶段未提供文件，仍会按统一查找规则回退（TypeID → fileBase → 类别 → default）
...
├─ bandage/
│  ├─ 64.mp3
│  └─ default.mp3           # （可选）bandage 通用回退
└─ syringe/
   ├─ needle_action.mp3
   ├─ needle_finish.mp3
   └─ default.mp3           # （可选）syringe 通用回退
```

### 常见使用场景

- 给“原本没有音效”的物品添加音效
  - 在 `items.{TypeID}` 下设置 `soundKey`，保持 `forceWhenNoEvent=true`
  - 放置对应类别的文件（或使用 `fileBase` 共享）
- 修正“原版分类不当”的物品
  - 设置 `items.{TypeID}.soundKey` 为期望的类别（例如将 meds 更正为 bandage）
  - 或使用 `actionKey`/`finishKey` 在两个阶段使用不同分类
- 让多个不同物品使用同一个音频
  - 为多个 TypeID 设置相同的 `fileBase`，并在对应类别目录下仅放置一份音频文件

> 注意：修改 `item_sound_map.json` 后，建议回到主菜单或重启游戏以确保生效。
