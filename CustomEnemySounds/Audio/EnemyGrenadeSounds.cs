...
namespace DuckovCustomSounds.CustomEnemySounds.Audio
{
    ...
    internal static class EnemyGrenadeSounds
    {
        ...
        // 敌人发现手雷的最大距离（米）- 超过此距离不会发出惊讶语音
        // 使用配置文件中的设置，默认为10.0f
        private static float MaxGrenadeNoticeDistance => DuckovCustomSounds.ModSettings.NPCGrenadeSurprisedMaxDistance;
        ...
        private static void AI_OnSound_Postfix(AICharacterController __instance, AISound sound)
        {
            try
            {
                ...
                // 额外距离检查：只有当敌人距离手雷足够近时才发出惊讶语音
                var actualDistance = Vector3.Distance(my, pos);
                if (actualDistance > MaxGrenadeNoticeDistance)
                {
                    CESLogger.Debug($"[GrenadeVoice] AI#{__instance.GetInstanceID()} 距离手雷过远 ({actualDistance:F1}m > {MaxGrenadeNoticeDistance}m)，不播放语音");
                    return;
                }
