...
namespace DuckovCustomSounds.CustomGunSounds
{
    ...
    /// </summary>
    internal static class GunConfig
    {
        private static readonly ModConfigScope Scope = ModConfigScopes.Gun;
            ...
            try
            {
                ...
                if (ModConfigAPI.IsAvailable())
                {
                    try
                    {
                        ...
                        LoadFromModConfig();
                        GunLogger.Debug("已集成 ModConfig UI");
                    }
                    catch (Exception ex)
                    {
                        // ModConfig 集成失败（意外情况），静默回退到默认配置
                        GunLogger.Debug($"ModConfig 集成异常（已回退到默认配置）: {ex.Message}");
                    }
                }
                else
                {
                    // ModConfig 不可用（正常兼容场景），静默使用默认配置
                    GunLogger.Debug("ModConfig 不可用，使用默认配置");
                }

                GunLogger.Info($"配置加载完成: Enabled={Enabled}, Volume={Volume:F2}");
            }
            catch (Exception ex)
            {
                GunLogger.Error("配置加载失败，使用默认配置", ex);
            }
        }
        ...
        {
            ModConfigAPI.SafeAddBoolDropdownList(Scope, "enabled", "启用自定义枪械音效", Enabled);
        ...
        private static void OnOptionsChanged(string key)
        {
            ...
            {
                GunLogger.Info($"配置已热重载: Enabled {oldEnabled} → {Enabled}");
            }

            // 如果音量变更，记录日志
            if (Math.Abs(oldVolume - Volume) > 0.01f)
            {
                GunLogger.Debug($"配置已热重载: Volume {oldVolume:F2} → {Volume:F2}");
            }
        }
    }
}
